from flask import render_template, url_for, flash, redirect, request, jsonify
from flask_api import app, db, bcrypt
from flask_api.forms import RegistrationForm, LoginForm
from flask_api.model import User
from flask_api.model import Invent
from flask_login import login_user, current_user, logout_user, login_required

@app.route("/register", methods=['GET', 'POST'])
def register():
    if current_user.is_authenticated:
        return jsonify('user_authenticated')
    form = RegistrationForm()
    if form.validate_on_submit():
        hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
        user = User(username=form.username.data, email=form.email.data, password=hashed_password)
        db.session.add(user)
        db.session.commit()
        flash('Your account has been created! You are now able to log in', 'success')
        return redirect(url_for('login'))
    return render_template('register.html', title='Register', form=form)


@app.route("/login", methods=['GET', 'POST'])
def login():
    if current_user.is_authenticated:
        return redirect(url_for('home'))
    form = LoginForm()
    if form.validate_on_submit():
        user = User.query.filter_by(email=form.email.data).first()
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            login_user(user, remember=form.remember.data)
            next_page = request.args.get('next')
            return redirect(next_page) if next_page else redirect(url_for('home'))
        else:
            flash('Login Unsuccessful. Please check email and password', 'danger')
    return render_template('login.html', title='Login', form=form)


@app.route("/accessory", methods = ['POST'])
def accessory():
    if request.method == 'POST':

        id = request.form.get("id","")
        accessory = request.form.get("accessory")
        quantity_ordered = request.form.get("quantity_ordered")
        quant_remaining  =request.form.get("quant_remaning")
        vendor_name = request.form.get("vendor_name")
        cp_accessory  = request.form.get("cp_accessory")
        sp_accessory = request.form.get("sp_accessory")
        details = Invent.query.all()


        db.session.commit()
        flash(f'Your product  has been updated!', 'success')
        return redirect('/Product')

    return jsonify('accessory added')


@app.route("/delete")
def delete():
    type = request.args.get('id')
    if type == 'accessory':
        pid = request.args.get('p_id')
        Invent = Invent.query.filter_by(prod_id=pid).delete()
        db.session.commit()
        flash(f'Your product  has been deleted!', 'success')
        return redirect(url_for('accessory'))
    return jsonify('accessory deleted')
